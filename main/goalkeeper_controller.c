...
goalkeeper_controller_handle_t *goalkeeper_controller_default_config(goalkeeper_controller_handle_t *handle)
{
        ...
        handle->GK_servo = &goalkeeper_servo;
        handle->state = STATE_GOALKEEPER_RESET;
        handle->GK_servo_angle = GOALKEEPER_DEFAULT_ANGLE;
        return handle;
}

void goalkeeper_controller_init(goalkeeper_controller_handle_t *handle)
{
        servo_default_config(handle->GK_servo);
        servo_init(handle->GK_servo, LEDC_TIMER_0, LEDC_CHANNEL_3, GPIO_SERVO_4);
        servo_set_angle(handle->GK_servo, GOALKEEPER_DEFAULT_ANGLE);
}
...
void goalkeeper_controller(goalkeeper_controller_handle_t *handle, button_event_t *event)
{
        ...
        case STATE_GOALKEEPER_LEFT:
                ...
                handle->GK_servo_angle = GOALKEEPER_TILT_LEFT_ANGLE;
                break;
        case STATE_GOALKEEPER_RIGHT:
                ...
                handle->GK_servo_angle = GOALKEEPER_TILT_RIGHT_ANGLE;
                break;
        default:
                break;
        }
